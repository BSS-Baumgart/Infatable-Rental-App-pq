...
export interface TranslationContextType {
  ...
  t: (key: string) => string;
  isLoading: boolean;
  formatT: (key: string, params?: Record<string, string | undefined>) => string;
}
...
export function TranslationProvider({ children }: { children: ReactNode }) {
  ...
  const [translations, setTranslations] =
    useState<Record<string, string>>(defaultTranslations);
  ...
  useEffect(() => {
    ...
    async function loadTranslations() {
      ...
      try {
        ...
        console.log(
          `Loaded ${Object.keys(data).length} translations for ${language}`
        );
  ...
  return (
    <TranslationContext.Provider
      value={{ language, setLanguage, t, isLoading, formatT }}
    >
      {children}
    </TranslationContext.Provider>
  );
